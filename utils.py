...
def generate_tree_buffers(tree_choices, device="cuda", *, minimal: bool = False):
    def custom_sort(lst):
        # sort_keys=[len(list)]
        sort_keys = []
    ...
    # Speed-critical: `list.index` inside nested loops makes this O(n^2) in Python.
    # Precompute a mapping from prefix path -> index in the sorted list for O(1) lookup.
    def _prefix_index(sorted_choices):
        out = {}
        for idx, path in enumerate(sorted_choices):
            out[tuple(path)] = idx
        return out
    ...
    with Timer("sort"):
        ...
        prefix_to_idx = _prefix_index(sorted_tree_choices)
    ...
    # Initialize depth_counts to keep track of how many choices have a particular depth
        ...
        for i in range(len(depth_counts)):
            for j in range(depth_counts[i]):
                ...
                for c in range(len(cur_tree_choice) - 1):
                    ancestor_idx.append(prefix_to_idx[tuple(cur_tree_choice[:c + 1])] + 1)
        ...
        if not minimal:
            ...
            for i in range(len(sorted_tree_choices)):
                ...
                retrieve_indice = []
                cur_key = tuple(cur_tree_choice)
                if cur_key in retrieve_paths:
                    continue
                else:
                    for c in range(len(cur_tree_choice)):
                        prefix = tuple(cur_tree_choice[:c + 1])
                        retrieve_indice.append(prefix_to_idx[prefix])
                        retrieve_paths.add(prefix)
    ...
    if not minimal:
        tree_buffers["tree_indices"] = tree_indices
        tree_buffers["retrieve_indices"] = retrieve_indices

    # Move the tensors in the dictionary to the specified device
    tree_buffers = {
        # clone() is redundant here; `.to(device)` already returns a new tensor on the target device.
        k: v.to(device)
